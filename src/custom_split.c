...
/*   Updated: 2024/01/22 18:59:45 by jesmunoz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char **ft_custom_split(char *string, char splitter, char flag)
{
    ...
    int trigger;
    int flag_count;
    split = malloc((ft_big_array_size(string, splitter, flag) + 1) * sizeof(char *));
    if (!split)
        return (NULL);
    i = 0;
    j = 0;
    ...
    while (i <= ft_strlen(string))
    {
        if (string[i] == flag)
            flag_count++;
        if (string[i] != splitter && flag_count % 2 == 0 && trigger == -1)
            trigger = i;
        else if ((string[i] == splitter || string[i] == '\0' || i == ft_strlen(string)) && flag_count % 2 == 0 && trigger >= 0)
        {
            split[j++] = ft_word_dup(string, trigger, i);
            if (!split[j - 1])
